...
class HubFeedItemTagFilter < ActiveRecord::Base
  ...
  def validate_filter_uniqueness
    filters_with_this_tag = self.class.where(:hub_id => self.hub_id, :feed_item_id => self.feed_item_id).includes(:filter).collect{|hfitf| hfitf.filter.relevant_tag_id == self.filter.relevant_tag_id}.flatten.uniq
